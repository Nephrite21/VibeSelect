...
public class WristRotationDetector : MonoBehaviour
{
    ...
    [Tooltip("주요 축으로 인식하기 위한 최소 비율 (다른 축 대비)")]
    [Range(1.2f, 5f)]
    public float dominantAxisRatio = 1.8f;
    ...
    bool IsDominantAxisClear(Vector3 axis, Vector3 dominantAxis)
    {
        ...
        float secondMaxComponent = 0f;

        // 최대값과 두 번째 최대값 찾기
        if (absAxis.x >= absAxis.y && absAxis.x >= absAxis.z)
        {
            maxComponent = absAxis.x;
            secondMaxComponent = Mathf.Max(absAxis.y, absAxis.z);
        }
        else if (absAxis.y >= absAxis.x && absAxis.y >= absAxis.z)
        {
            maxComponent = absAxis.y;
            secondMaxComponent = Mathf.Max(absAxis.x, absAxis.z);
        }
        else
        {
            maxComponent = absAxis.z;
            secondMaxComponent = Mathf.Max(absAxis.x, absAxis.y);
        }

        // 최대 성분이 두 번째로 큰 성분보다 dominantAxisRatio배 이상 큰지 확인
        if (secondMaxComponent < 0.001f) // 0으로 나누기 방지
        {
            return true;
        }

        return (maxComponent / secondMaxComponent) >= dominantAxisRatio;
    }
